...
export interface ActiveBoundRuntimeService {
	...
	verifyActiveCancellation(tuple: { requestId: string; ownerRunId: string; nodeId: string }, binding: ActiveBoundBindingV1): boolean;
	claimBase(proof: ActiveBoundExecutionProofV1, identity: ActiveBoundRootIdentityV1, created: boolean): boolean;
	releaseBase(proof: ActiveBoundExecutionProofV1, identity: ActiveBoundRootIdentityV1): boolean;
...
export function activeBoundRequestFromDelegation(
	...
	const parsed = parseActiveBoundPreflightRequest(candidate);
	if (!parsed.ok) throw new Error("Delegated active-bound request is invalid.");
...
export function createActiveBoundRuntimeService(options: CreateActiveBoundRuntimeServiceOptions): ActiveBoundRuntimeService {
	...
	const ownedFutureBaseCapacity = 8_192;
	const runtimeMaxSubagentDepth = (): number => options.maxSubagentDepth ?? resolveCurrentMaxSubagentDepth(options.config.maxSubagentDepth);
	const runtimeForegroundTimeoutMs = (): number => {
	...
	const resolve = (request: ActiveBoundPreflightRequestV1, ownedRootIdentity?: ActiveBoundRootIdentityV1, ownedSessionDirIdentity?: ActiveBoundRootIdentityV1, ownedBaseRootIdentity?: ActiveBoundRootIdentityV1, projectOwnedBaseAsFuture = false): ResolveActiveBoundLaunchContractResult => {
		...
		try {
			...
			return resolveActiveBoundLaunchContract({
				...
				runtimePolicy: {
					foregroundTimeoutMs: runtimeForegroundTimeoutMs(),
	...
	const verifyCancellation = (tuple: { requestId: string; ownerRunId: string; nodeId: string }, binding: ActiveBoundBindingV1, enforceAdmissionLifetime: boolean): boolean => {
		if (disposed || binding.version !== 1 || binding.targetServerInstanceId !== options.serverInstanceId
			...
			|| (enforceAdmissionLifetime ? !receipts.verify(binding.receipt) || !receipts.verifyCancellation(binding.cancellationToken) : !receipts.verifyCancellationAuthenticity(binding.cancellationToken))) return false;
	...
	return {
		...
		verifyPendingCancellation(tuple, binding) { return verifyCancellation(tuple, binding, true); },
		verifyActiveCancellation(tuple, binding) { return verifyCancellation(tuple, binding, false); },
		admit(request, binding) {
			if (!verifyCancellation(request, binding, true) || disposed || binding.version !== 1 || binding.targetServerInstanceId !== options.serverInstanceId
		...
		claimBase(proof, identity, created) {
			...
			if (!ownedFutureBaseIdentity) {
				if (!created || ownedFutureBaseIdentities.size >= ownedFutureBaseCapacity) return false;
		...
		releaseBase(proof, identity) {
			if (disposed || proof.version !== 1 || proof.contract.roots.baseRootIdentityDigest !== undefined) return false;
			const key = proof.contract.roots.baseRootPathDigest;
			const owned = ownedFutureBaseIdentities.get(key);
			if (!owned || owned.dev !== identity.dev || owned.ino !== identity.ino) return false;
			return ownedFutureBaseIdentities.delete(key);
		},
