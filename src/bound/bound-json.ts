...
/** Clone plain JSON data without invoking getters, toJSON hooks, or Proxy traps. */
export function cloneJsonWithinByteLimit(input: unknown, maxBytes: number): BoundedJsonClone {
	...
	const visit = (value: unknown, depth: number): unknown => {
		...
		// Fork-only divergence from upstream src/slash/delegation-json.ts: a Proxy is
		// rejected before any own-key inspection, so none of its traps observes the input.
		if (utilTypes.isProxy(object)) throw new TypeError("invalid");
