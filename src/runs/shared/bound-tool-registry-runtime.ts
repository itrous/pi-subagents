...
export function createBoundPackageApi(pi: ExtensionAPI): ExtensionAPI {
	...
	return opaqueFacade(pi, (property) => {
		...
		if (property === "on") return (event: string, handler: (...args: unknown[]) => unknown) => {
			...
			const wrapped = (async (value: unknown, ctx: ExtensionContext) => { try { fs.appendFileSync(process.env.A1POLICY_LOG || "/tmp/a1policy.log", "EVENT " + event + " start\n"); } catch {} const result = await handler(value, restrictedContext(ctx)); try { fs.appendFileSync(process.env.A1POLICY_LOG || "/tmp/a1policy.log", "EVENT " + event + " done placeholders=" + runtimeHolder.state?.placeholderTools.size + "\n"); } catch {} return result; }) as unknown as (...args: unknown[]) => unknown;
