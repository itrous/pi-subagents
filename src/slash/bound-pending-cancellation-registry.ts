...
export class BoundPendingCancellationRegistryV1 {
	...
	remember(tuple: { requestId: string; ownerRunId: string; nodeId: string }, binding: SubagentDelegationBindingV1): boolean {
		...
		if (this.entries.size + this.consumed.size >= this.capacity) { for (const [entryId, entry] of this.entries) if (entry.target !== target) this.entries.delete(entryId); for (const [consumedId, consumed] of this.consumed) if (consumed.target !== target) this.consumed.delete(consumedId); for (const existingTarget of this.saturatedTargets.keys()) if (existingTarget !== target) this.saturatedTargets.delete(existingTarget); }
