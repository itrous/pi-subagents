...
import { parseSubagentDelegationBinding, parseSubagentDelegationRequest, subagentDelegationBindingTarget } from "./delegation-request.ts";
import {
...
interface PromptTemplateBridgeOptions<Ctx extends { cwd?: string }> {
	...
	pendingCancellationRegistry?: BoundPendingCancellationRegistryV1;
}
...
export function registerPromptTemplateDelegationBridge<Ctx extends { cwd?: string }>(
	...
	const pendingCancellations = options.activeBoundRuntime ? options.pendingCancellationRegistry ?? getBoundPendingCancellationRegistry() : undefined;
	const runtimeId = options.runtimeId ?? randomUUID();
	const subscriptions: Array<() => void> = [];
	const pendingLegacyCancellations = new Set<string>();
	let pendingLegacySaturated = false;
	const legacyTupleKey = (tuple: { requestId: string; ownerRunId: string; nodeId: string }) => JSON.stringify([tuple.requestId, tuple.ownerRunId, tuple.nodeId]);
	let active = false;
	let stopped = false;
	...
	subscribe(PROMPT_TEMPLATE_SUBAGENT_CANCEL_EVENT, (data) => {
		...
		const inspected = cloneJsonWithinByteLimit(data, 64 * 1024);
		if (!inspected.ok || !inspected.value || typeof inspected.value !== "object" || Array.isArray(inspected.value)) return;
		...
		if (keys === "nodeId,ownerRunId,requestId") {
			if (!coordinator.cancel(tuple.requestId, tuple.ownerRunId, tuple.nodeId, "legacy")
				&& coordinator.canRememberCancellation(tuple.requestId, tuple.ownerRunId, tuple.nodeId)) {
				const id = legacyTupleKey(tuple as { requestId: string; ownerRunId: string; nodeId: string });
				if (pendingLegacyCancellations.size >= 8_192) pendingLegacySaturated = true;
				else pendingLegacyCancellations.add(id);
			}
			return;
		}
		...
		const exactTuple = tuple as { requestId: string; ownerRunId: string; nodeId: string };
		if (!binding || binding.targetServerInstanceId !== value.targetServerInstanceId
			|| !options.activeBoundRuntime.verifyActiveCancellation(exactTuple, binding)) return;
		...
		if (options.activeBoundRuntime.verifyPendingCancellation(exactTuple, binding)
			&& coordinator.canRememberCancellation(tuple.requestId, tuple.ownerRunId, tuple.nodeId)) pendingCancellations.remember(exactTuple, binding);
	});
	...
	function executeStructured(request: SubagentDelegationRequest, ownBindingProperty = request.binding !== undefined): void {
		if (!active || stopped) return;
		if (!request.binding) {
			const pendingId = legacyTupleKey(request);
			if (pendingLegacyCancellations.delete(pendingId) || pendingLegacySaturated) {
				coordinator.commitRejected(request, runtimeId, {
					requestId: request.requestId, ownerRunId: request.ownerRunId, nodeId: request.nodeId,
					status: pendingLegacySaturated ? "unavailable_context" : "cancelled",
					...(pendingLegacySaturated ? { error: "Pending legacy cancellation capacity is exhausted for this runtime." } : {}),
				});
				return;
			}
		}
		...
		if (request.binding && pendingCancellations) {
			...
			if (pending) {
				if (pending === "saturated") {
					if (prospectiveReservation) boundRegistry!.release(prospectiveReservation.serverInstanceId, prospectiveReservation.prospectiveRunId);
					admission.settle({ requestId: request.requestId, ownerRunId: request.ownerRunId, nodeId: request.nodeId, status: "unavailable_context", error: "Pending cancellation capacity is exhausted for this runtime generation." });
					return;
				}
	...
	return {
		...
		dispose: () => {
			...
			pendingLegacyCancellations.clear();
