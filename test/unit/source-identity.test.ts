...
describe("active runtime source identity", { skip: process.platform !== "linux" }, () => {
	...
	it("attests a clean detached fork while ignoring untracked files", () => {
		const repo = fixture();
		try {
			fs.writeFileSync(path.join(repo.root, "untracked"), "ignored\n");
	...
	it("ignores bogus fsmonitor and inherited Git discovery environment", () => {
		...
		try {
			...
			const result = resolveActiveRuntimeSourceIdentity({ packageRoot: repo.root });
			assert.equal(result.available, true);
			assert.deepEqual(gitIdentityEnvironment({ PATH: "/bin", Git_DIR: "x" }), { PATH: "/bin", GIT_NO_REPLACE_OBJECTS: "1", LC_ALL: "C" });
		} finally {
	...
	it("rejects malformed ls-tree without a terminating NUL", () => {
		...
		try {
			...
			const runner = (_root: string, phase: "initial" | "final", commit: string | undefined, limits: { timeoutMs: number; outputBytes: number }) => {
				...
				return { ...real, commands: real.commands.map((entry, index) => index === 5 ? { ...entry, stdout: entry.stdout.subarray(0, -1) } : entry) };
			};
	...
	it("kills descendants left by an earlier completed Git command", async () => {
		...
		try {
			...
			fs.writeFileSync(path.join(bin, "git"), [
				"#!/bin/sh",
				"case \" $* \" in",
				"  *' rev-parse --show-toplevel '*) (sleep 30) >/dev/null 2>&1 & echo $! > \"$PIDFILE\"; exec \"$REAL_GIT\" \"$@\" ;;",
				"  *) sleep 30 ;;",
				"esac",
			].join("\n"), { mode: 0o755 });
			...
			assert.equal(runGitProbe(repo.root, "initial", undefined, { timeoutMs: 300, outputBytes: 4 * 1024 * 1024 }).timedOut, true);
			...
			if (oldPidFile === undefined) delete process.env.PIDFILE; else process.env.PIDFILE = oldPidFile;
	...
	it("bounds the whole Git process tree", () => {
		...
		try {
			...
			process.env.PATH = `${bin}${path.delimiter}${oldPath ?? ""}`;
			const started = Date.now();
			assert.equal(runGitProbe(process.cwd(), "initial", undefined, { timeoutMs: 2_000, outputBytes: 4 * 1024 * 1024 }).timedOut, true);
