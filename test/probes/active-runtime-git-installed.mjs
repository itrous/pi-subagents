...
async function runRegistryStopGate() {
	...
	const policy = { version: 1, modelApi: "openai-completions", piRuntimeVersion: piVersion, proofNonce, denialFd: 4, required: ["missing_probe_tool"], internalTools: [], runtimeExtensions: evidenceModule.attestBoundRuntimeExtensions(runtimeExtensionPaths), packageExtensions: [] };
	...
	assert.ok(fs.statSync(proofPath).size <= 64 * 1024, "negative registry proof exceeded 64 KiB"); const frame = JSON.parse(fs.readFileSync(proofPath, "utf8")); assert.equal(frame.kind, "registry"); assert.deepEqual(frame.projection.required, ["missing_probe_tool"]); assert.deepEqual(frame.projection.effectiveCallerTools, ["read"]); assert.deepEqual(frame.projection.internalTools, []); assert.deepEqual(frame.projection.missing, ["missing_probe_tool"]);
	const digest = createHash("sha256").update('{"effectiveCallerTools":["read"],"internalTools":[],"missing":["missing_probe_tool"],"projectionVersion":1,"required":["missing_probe_tool"],"version":1}').digest("hex"); assert.equal(frame.projection.digest, digest);
