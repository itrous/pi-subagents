...
- В1 из плана A1R.4 (решение Р12): явный путь к конфигурации pi-mcp-adapter для листа
  при `process.cwd() !== ctx.cwd` вместо закрытого отказа Д10; конфигурация адаптера
  становится входом контракта v2.
...
- **Р12 (2026-09-21), вилка В1 плана A1R.4.** Закрытый отказ Д10 для MCP-листа при
  `process.cwd() !== ctx.cwd` остаётся; явная передача пути к конфигурации адаптера
  (конфигурация — вход контракта v2) переносится в A1R.6. Замер —
  `LANDING-A1R.5-installed-probe.md`.
